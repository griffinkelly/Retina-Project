...
[win, windowRect] = Screen('OpenWindow', screenid, 128);
...
gratingtex = CreateProceduralSquareWaveGrating(win, res(1), res(2), [0.5 0.5 0.5 0.0]);
...
circleDiameter = max(baseRect) * 5.00;
...
while keepdisplay
        ...
        ovalCenteredRectangle = CenterRectOnPointd(ovalBaseRect, xCenter, yCenter);

        corner = CenterRectOnPointd(baseSquare, screenXpixels, screenYpixels);

        Screen('DrawTexture', win, gratingtexSmall, [], centeredRect(1,:), angle, [], [], [], [], rotateMode, [phase2, freq, amplitude, 0]);
        Screen('FrameOval', win, [128 128 128], ovalCenteredRectangle, 2, 50 );
...
%%		vbl = Screen('Flip', win, vbl + 0.5 * ifi);
    ...
    %switch phases ever 2 seconds
    if (mod(floor(telapsed/4),2)==0)
        phase2 = -phase;
    else
        phase2 =phase;
    end
