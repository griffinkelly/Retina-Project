...
%		[keydown, secs, keycode, deltasexcs] = KbCheck;	
	if keycode(exitkey)
		Screen('CloseAll');
		str = sprintf('%d, %d, %d, %f', angle, cyclespersecond, gratingwidth, risetime);
		disp(str);
		break
...
%				if counter_in > usb_counter, break, end
			end	
			WaitSecs(5);
			tstart1 = GetSecs;
			DaqAOut(device, analogChannel0, onVolts);
			risetime = GetSecs - tstart1;
			Screen('FillRect', win, 128);
%			risetime = GetSecs - tstart1;
			Screen(win, 'Flip');
			WaitSecs(LEDontime);
			DaqAOut(device, analogChannel0, offVolts);
			Screen('FillRect', win, black);
			Screen(win, 'Flip');
			WaitSecs(LEDofftime);
%		KbReleaseWait;
