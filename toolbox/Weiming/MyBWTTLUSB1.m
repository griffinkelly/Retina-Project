function MyBWTTL(numRects, rectSize, scale, syncToVBL, dontclear)
% MyBWTTL([numRects=1][, rectSize=128][, scale=1][, syncToVBL=1][, dontclear=0])
%
...
increaseframedelay = KbName('1');
decreaseframedelay = KbName('0');
...
try
	...
	refreshrate = Screen('FrameRate', screenid);
	Screen('Resolution', screenid, screenResolution(1), screenResolution(2), refreshrate);
...
%	Screen('Resolution', win, [], [], refreshrate);
%	Screen('ConfigureDisplay', 0, 0, 0, [], [], refreshrate);
%	Screen('Resolution', screenid, screenResolution(1), screenResolution(2), refreshrate);
...
%			noiseimg(:, :, 1) = int_amplitude*(floor(2*rand(rectSize, rectSize)));
%			noiseimg(:, :, 2) = int_amplitude*(floor(2*rand(rectSize, rectSize)));
%			noiseimg(:, :, 3) = int_amplitude*(floor(2*rand(rectSize, rectSize)));
...
%			noiseimg(:, :, 1) =uint8(int_amplitude*(round(rand(rectSize, rectSize))*255));

			noiseimg1 = int_amplitude*(floor(rand(rectSize, rectSize)));
			noiseimg2 = int_amplitude*(ceil(rand(rectSize, rectSize))) + 254;
...
%           tex=Screen('MakeTexture', win, noiseimg);
			...
			tex1=Screen('MakeTexture', win, noiseimg1);
%			tex2=Screen('MakeTexture', win, noiseimg2);
			...
			Screen('DrawTexture', win, tex1, [], dstRect(i,:), [], 0);			
%			Screen(win, 'Flip');
			Screen('Flip', win, 0, dontclear, asyncflag);
%			count = count + 1;
%			Screen('DrawTexture', win, tex2, [], dstRect(i,:), [], 0);			

%			Screen('DrawTexture', win, tex, [], dstRect(i,:), [], 0);
%			Screen('DrawTexture', win, tex, [], dstRect(i,:), [], 0, [0: 255: 0]);
%			if numFramedelay ~=0
				Screen('WaitBlanking', win, numFramedelay);
%				Screen('WaitBlanking', win, 100);
%			end
			tex2=Screen('MakeTexture', win, noiseimg2);
			Screen('DrawTexture', win, tex2, [], dstRect(i,:), [], 0);
%			Screen(win, 'Flip'); 
...
%		KbReleaseWait;
  	if keycode(exitkey)
		...
		Screen('CloseAll');
		str = sprintf('%d, %d, %d, %d, %.2f, %.2f, %.2f, %f', int_amplitude, rectSize, scale, count, telapsed, updaterate, 60/(numFramedelay+1), delay_time);
%		str = sprintf('%d, %d, %d', int_amplitude, rectSize, scale);
		disp(sprintf("	counter	  running time	framerate(avg) refresh rate(current) trigger(delay)"));
		disp(str);
...
%			Screen('DrawTexture', win, tex, [], dstRect(i,:), [], 0);
	...
	elseif keycode(increaseframedelay)
		numFramedelay = numFramedelay + 1;
	elseif keycode(decreaseframedelay)
		numFramedelay = numFramedelay - 1;
		if numFramedelay < 0
			numFramedelay = 0;
		end
	elseif keycode(blackkey)
		...
		err = DaqCInit(device); 
		usb_counter = DaqCIn(device);
			while usb_counter == 0
%			while ~KbCheck
%				pause();
				counter_in = DaqCIn(device);
				usb_counter = counter_in;
				if KbCheck, break, end
			end
		trigger_time = GetSecs;
		Screen('FillRect', win, 128);
		Screen(win, 'Flip');
		delay_time = GetSecs - trigger_time;    	
