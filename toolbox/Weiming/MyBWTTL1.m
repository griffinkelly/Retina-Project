...
%		KbReleaseWait;
  	if keycode(exitkey)
		...
		Screen('CloseAll');
		str = sprintf('%d, %d, %d, %d, %.2f, %.2f, %.2f', int_amplitude, rectSize, scale, count, telapsed, updaterate, 60/(numFramedelay+1));
%		str = sprintf('%d, %d, %d', int_amplitude, rectSize, scale);
		disp(sprintf("	counter	  running time	framerate(avg) refresh rate(current)"));
		disp(str);
